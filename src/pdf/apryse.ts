...
export const apryseRouter = express.Router();

// Serve WebViewer assets (after installing the package)
apryseRouter.use('/webviewer', express.static(path.join(process.cwd(), 'node_modules/@pdftron/webviewer/public')));

/**
 * Configure document viewing with Apryse WebViewer
 * @param app Express application
 */
export function setupApryseViewer(app: express.Application): void {
    logger.info('Setting up Apryse PDF viewer integration');

    // Register the Apryse router
    app.use('/pdf', apryseRouter);

    // Endpoint to get WebViewer configuration for a specific document
    apryseRouter.get('/viewer/:id', async (req, res) => {
        try {
            ...
            const doc = docs[0];
            const userName = req.user ? (req.user as any).username : 'Guest';
...
<body>
  ...
  <script>
      ...
      UI.setHeaderItems(header => {
        ...
        header.push({
          ...
          onClick: async () => {
            ...
            logger.error('Error generating PDF viewer:', error);
            res.status(500).send('Error generating PDF viewer');
        }
    });

    // Endpoint to save annotations
    apryseRouter.post('/documents/:id/annotations', async (req, res) => {
        try {
            const documentId = req.params.id;
            ...
            await db.query(
                `INSERT INTO document_annotations (document_id, user_id, annotations, created_at)
                 VALUES (?, ?, ?, ?)
            ...
            logger.error('Error saving annotations:', error);
            res.status(500).json({ error: 'Failed to save annotations' });
        }
    });

    // Endpoint to get annotations
    apryseRouter.get('/documents/:id/annotations', async (req, res) => {
        try {
            const documentId = req.params.id;
            const db = getConnection();

            const results = await db.query(
                'SELECT * FROM document_annotations WHERE document_id = ?',
                [documentId]
            );
    ...
    apryseRouter.post('/documents/:id/ocr', async (req, res) => {
        try {
            const documentId = req.params.id;
            const db = getConnection();
 ...
 */
async function processOcr(filePath: string, documentId: number): Promise<void> {
    ...
    logger.info(`OCR processing started for document ${documentId} at ${filePath}`);
