...
folderRouter.get('/', isAuthenticated, async (req, res) => {
    try {
...
// Create a new folder
folderRouter.post('/', isAuthenticated, async (req, res) => {
    try {
        ...
        const existingFolders = await db.query(
            'SELECT id FROM folders WHERE name = ? AND parent_id IS ? AND created_by = ?',
            [name, parent_id || null, currentUser.id]
        );
        ...
        if (parent_id) {
            const parentFolders = await db.query(
                'SELECT id FROM folders WHERE id = ? AND created_by = ?',
        ...
        const result = await db.query(
            ...
            [name, parent_id || null, currentUser.id, new Date()]
        );
...
folderRouter.get('/:id', isAuthenticated, async (req, res) => {
    try {
...
// Update folder
folderRouter.put('/:id', isAuthenticated, async (req, res) => {
    try {
        const folderId = parseInt(req.params.id);
        const { name, parent_id } = req.body;
        const currentUser = req.user as any;
        ...
        if (parent_id && parent_id !== folder.parent_id) {
            ...
            const checkCircular = async (checkId: number, targetId: number): Promise<boolean> => {
                if (checkId === targetId) {
                    return true;
                }
        ...
        if (name && name !== folder.name) {
            // Check if folder with same name exists at same level
            const existingFolders = await db.query(
                'SELECT id FROM folders WHERE name = ? AND parent_id IS ? AND created_by = ? AND id != ?',
                [name, parent_id || folder.parent_id || null, currentUser.id, folderId]
            );
...
// Delete folder
folderRouter.delete('/:id', isAuthenticated, async (req, res) => {
    try {
        const folderId = parseInt(req.params.id);
        const currentUser = req.user as any;
...
folderRouter.post('/:id/documents/:documentId', isAuthenticated, async (req, res) => {
    try {
        const folderId = parseInt(req.params.id);
        ...
        const documents = await db.query(
            'SELECT * FROM documents WHERE id = ? AND (uploaded_by = ? OR id IN (SELECT document_id FROM document_shares WHERE shared_with = ?))',
        ...
        const existing = await db.query(
            'SELECT * FROM document_folders WHERE document_id = ? AND folder_id = ?',
            [documentId, folderId]
        );
        ...
        logger.error(`Error adding document ${req.params.documentId} to folder ${req.params.id}:`, error);
        res.status(500).json({ error: 'Failed to add document to folder' });
    }
});

// Remove document from folder
folderRouter.delete('/:id/documents/:documentId', isAuthenticated, async (req, res) => {
    try {
        const folderId = parseInt(req.params.id);
        const documentId = parseInt(req.params.documentId);
        const currentUser = req.user as any;
        ...
        logger.error(`Error removing document ${req.params.documentId} from folder ${req.params.id}:`, error);
