...
export const caseDashboardRouter = express.Router();

// Get case dashboard summary
caseDashboardRouter.get('/summary', isAuthenticated, async (req, res) => {
    try {
        const perfEnd = performance.start('case-dashboard-summary');
    ...
    `, [userId, userId]);
        ...
        const deadlines = await getUpcomingDeadlines(14); // Get deadlines for next 14 days
    ...
    `;
        ...
        logger.info(`Generated case dashboard summary in ${perfEnd()}ms`);
        res.json(result);

    } catch (error) {
        logger.error('Error generating case dashboard summary:', error);
        res.status(500).json({ error: 'Failed to generate dashboard summary' });
    }
});

// Get case timeline events
caseDashboardRouter.get('/timeline', isAuthenticated, async (req, res) => {
    try {
...
caseDashboardRouter.post('/scan-documents-for-events', isAuthenticated, async (req, res) => {
    try {
        const db = getConnection();
        const userId = (req.user as any).id;
    ...
    `, [userId, userId]);
        ...
        for (const doc of documents) {
            ...
            const { extractTimelineFromDocument } = await import('../case/timelineExtractor');
        ...
        logger.error('Error scanning documents for events:', error);
        res.status(500).json({ error: 'Failed to scan documents' });
    }
});

// Get document similarity analysis
caseDashboardRouter.get('/document-similarity/:id', isAuthenticated, async (req, res) => {
    try {
    ...
    `, [documentId, documentId, userId, userId]);
        ...
        logger.error(`Error analyzing document similarity for document ${req.params.id}:`, error);
        res.status(500).json({ error: 'Failed to analyze document similarity' });
    }
});

// Get case insights
caseDashboardRouter.get('/insights', isAuthenticated, async (req, res) => {
    try {
        const db = getConnection();
        const userId = (req.user as any).id;
    ...
    `, [userId, userId]);
        ...
        logger.error('Error generating case insights:', error);
