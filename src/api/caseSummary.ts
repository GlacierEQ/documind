...
import { summarizeDocument } from '../ai/processor';
import { getCaseTimeline } from '../case/timelineExtractor';
import { getUpcomingDeadlines } from '../case/deadlineTracker';

export const caseSummaryRouter = express.Router();

// Get AI-generated case summary
caseSummaryRouter.get('/summary', isAuthenticated, async (req, res) => {
    try {
        const perfEnd = performance.start('case-summary-generation');
    ...
    `;
        ...
        // Get timeline events
        const timelineEvents = await getCaseTimeline(userId, 90); // Last 90 days of events

        // Get upcoming deadlines
        const deadlines = await getUpcomingDeadlines(30); // Next 30 days of deadlines

        // Check for cache first before generating summary
        const cachedSummary = await db.query(
            'SELECT * FROM case_summaries WHERE user_id = ? AND created_at > ?',
            [userId, new Date(Date.now() - 24 * 60 * 60 * 1000)] // Last 24 hours
        );
        ...
        const entitiesByType = entities.reduce((acc: any, entity: any) => {
        ...
        logger.info(`Generated case summary in ${perfEnd()}ms`);
 ...
 */
async function generateCaseSummary(documents: any[], entities: any[], timelineEvents: any[], deadlines: any[]) {
    const config = loadConfig();
    ...
    `;
        ...
        const summaryText = await summarizeDocument(prompt, config.ai.maxTokens || 2000);
        ...
        try {
            ...
            logger.error('Error parsing AI summary response:', parseError);
        ...
        logger.error('Error generating AI case summary:', error);
 ...
 */
function createSummaryContext(documents: any[], entities: any[], timelineEvents: any[], deadlines: any[]) {
    ...
    if (entities.length > 0) {
        ...
        const importantEntities = entities.filter((e: any) => e.importance >= 7);
        importantEntities.forEach((entity: any) => {
 ...
 */
function extractListItems(text: string, listName: string): string[] {
    // Find the section
    ...
    if (bulletItems.length > 1) return bulletItems.map(item => item.trim());
