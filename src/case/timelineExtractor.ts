 ...
 */
export async function extractTimelineFromDocument(documentId: number, text: string): Promise<TimelineEvent[]> {
    if (!text) {
        logger.warn(`No text provided for timeline extraction, document ID: ${documentId}`);
        return [];
    }

    const perfEnd = performance.start('timeline-extraction');
    ...
    try {
            ...
            for (const pattern of eventDef.patterns) {
                ...
                while ((match = pattern.exec(normalizedText)) !== null) {
                    try {
                        ...
                        const parsedDates = chrono.parse(dateString);
                        ...
                        logger.warn(`Error parsing date in timeline extraction: ${match[1]}`, error);
                    }
                }
            }
        }
        ...
        for (const dateStr of allMatches) {
            try {
                const parsedDates = chrono.parse(dateStr);
                ...
                logger.warn(`Error parsing additional date in timeline extraction: ${dateStr}`, error);
            }
        }
        ...
        for (const event of events) {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
                ...
                logger.error(`Error saving timeline event to database:`, error);
            }
        }

        logger.info(`Extracted ${events.length} timeline events in ${perfEnd()}ms from document ${documentId}`);
        return events;

    } catch (error) {
        logger.error(`Error extracting timeline from document ${documentId}:`, error);
        return [];
    }
}
 ...
 */
export async function getCaseTimeline(userId: number, days: number = 365): Promise<TimelineEvent[]> {
    const db = getConnection();
