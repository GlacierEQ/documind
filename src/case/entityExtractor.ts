...
import { summarizeDocument, analyzeDocument } from '../ai/processor';
 ...
 */
export async function findReferences(text: string): Promise<DocumentReference[]> {
    ...
    try {
        ...
        logger.error('Error finding document references:', error);
        return [];
    }
}
 ...
 */
function extractEntitiesRuleBased(text: string): Entity[] {
    const entities: Entity[] = [];
    ...
    if (nameMatches) {
        nameMatches.forEach(match => {
            ...
            if (foundEntities.has(match)) return;
