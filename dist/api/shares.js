...
exports.sharesRouter = express_1.default.Router();
// Get all shares for current user (both shared by user and shared with user)
exports.sharesRouter.get('/', auth_1.isAuthenticated, async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching shares:', error);
        res.status(500).json({ error: 'Failed to fetch shares' });
    }
});
// Share a document with another user
exports.sharesRouter.post('/', auth_1.isAuthenticated, async (req, res) => {
    try {
        ...
        const documents = await db.query('SELECT * FROM documents WHERE id = ? AND uploaded_by = ?', [documentId, currentUser.id]);
        ...
        const existingShares = await db.query('SELECT id FROM document_shares WHERE document_id = ? AND shared_with = ?', [documentId, sharedWithId]);
        if (existingShares && existingShares.length > 0) {
            // Update existing share instead of creating a new one
            await db.query('UPDATE document_shares SET permission = ?, expires_at = ? WHERE document_id = ? AND shared_with = ?', [permission, expiresAt, documentId, sharedWithId]);
            return res.json({
                success: true,
        ...
        // Create share
        const result = await db.query('INSERT INTO document_shares (document_id, shared_by, shared_with, permission, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?)', [documentId, currentUser.id, sharedWithId, permission, new Date(), expiresAt]);
...
// Delete a share
exports.sharesRouter.delete('/:id', auth_1.isAuthenticated, async (req, res) => {
    try {
        const shareId = parseInt(req.params.id);
        const currentUser = req.user;
        const db = (0, connection_1.getConnection)();
        // Check if share exists and belongs to user
        const shares = await db.query('SELECT * FROM document_shares WHERE id = ? AND shared_by = ?', [shareId, currentUser.id]);
        ...
        logger_1.logger.error(`Error deleting share ${req.params.id}:`, error);
