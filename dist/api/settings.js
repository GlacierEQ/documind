...
exports.settingsRouter.use(auth_1.isAdmin);
...
exports.settingsRouter.put('/', async (req, res) => {
    try {
        ...
        let envContent;
        try {
            envContent = await promises_1.default.readFile(envFilePath, 'utf8');
        }
        catch (err) {
            logger_1.logger.warn(`No .env file found at ${envFilePath}, creating new one`);
            envContent = '';
        }
        // Parse current .env file
        const currentEnv = dotenv_1.default.parse(envContent);
        ...
        await promises_1.default.writeFile(envFilePath, envFileContent);
...
exports.settingsRouter.post('/ai', isAuthenticated, auth_1.isAdmin, async (req, res) => {
    try {
        const { provider, maxTokens, temperature, openaiApiKey, openaiModel, openaiEmbeddingModel, azureApiKey, azureEndpoint, azureDeploymentName, anthropicApiKey, anthropicModel, cohereApiKey, cohereModel, localModelPath, localModelConfig, 
        // Add Granite settings
        graniteApiKey, graniteModel, graniteEmbeddingModel } = req.body;
        // Basic validation
        if (!provider) {
